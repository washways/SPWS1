            ...
            if (show) {
                ...
                if (!geeLayersRef.current[type]) {
                    ...
                    try {
                        ...
                        const res = await fetch(`http://localhost:3001/api/get_gee_layer?type=${type}`);

                        if (res.status === 503) {
                            alert(`GEE Authentication Missing.\n\nPlease add 'gee-key.json' (Service Account Key) to the root folder to enable ${name}.`);
                        ...
                        if (data.success && data.urlFormat) {
                            // Add Tile Layer
                            const layer = L.tileLayer(data.urlFormat, {
                                attribution: 'Google Earth Engine & Partners',
                                opacity: 0.7
                            }).addTo(mapInstanceRef.current!);

                            geeLayersRef.current[type] = layer;
                        } else {
                            throw new Error(data.error || 'Failed to get Map ID');
                        }

                    } catch (e) {
                        console.error("GEE Fetch Error", e);
                        alert(`Failed to load ${name}. Backend might not be running or key is invalid.`);
                        if (type === 'dtw') setShowDTW(false);
                        if (type === 'gw') setShowGWPotential(false);
                        if (type === 'dem') setShowFABDEM(false);
                    }
