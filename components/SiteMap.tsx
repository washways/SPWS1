...
import { Map as MapIcon, Navigation, Trash2, Settings, CheckCircle, Layers, Disc, Box, Spline, CircleDot, Activity, MousePointerClick, User, Users, Eraser, Search, FileText, Hash, GraduationCap, Stethoscope, Sprout, Zap, Mountain } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 h-[calc(100vh-140px)] relative">
      ...
      <div className="order-2 md:order-1 w-full md:w-80 flex flex-col gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-200 overflow-y-auto max-h-[40%] md:max-h-full">
         <div className="mb-2">
            <h3 className="font-bold text-gray-900 flex items-center gap-2"><Settings className="w-4 h-4"/> Design Parameters</h3>
         </div>
         {/* Alert Prompt */}
         <div className="bg-amber-50 border border-amber-200 rounded p-2 text-xs text-amber-800 mb-2 flex items-start gap-2">
            <Activity className="w-3 h-3 mt-0.5 flex-shrink-0" />
            <span><strong>Important:</strong> Adjust these values based on actual site survey data for accurate results.</span>
         </div>
         
         <div className="space-y-3 p-3 bg-slate-50 rounded-lg border border-slate-100">
             <div className="space-y-1">
                 <label className="text-xs font-medium text-gray-500">Site Name</label>
                 <input type="text" value={projectDetails.siteName} onChange={e => setProjectDetails({...projectDetails, siteName: e.target.value})} className="w-full text-sm p-1.5 border rounded" placeholder="e.g. Mwanza East"/>
             </div>
             <div className="space-y-1">
                 <label className="text-xs font-medium text-gray-500">Contract No.</label>
                 <input type="text" value={projectDetails.contractNumber} onChange={e => setProjectDetails({...projectDetails, contractNumber: e.target.value})} className="w-full text-sm p-1.5 border rounded" placeholder="e.g. 021/MW/24"/>
             </div>
         </div>
         <form onSubmit={handleSearch} className="relative">
             <input type="text" placeholder="Search Location..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
